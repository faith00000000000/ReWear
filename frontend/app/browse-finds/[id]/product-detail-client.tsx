...
// }
//
// // Deterministic placeholder so it doesn't shuffle on every render
...
//
//   // Rent calendar local state (placeholder — swap for real availability later)
//   const today = useMemo(() => new Date(), []);
//   const [calYear, setCalYear] = useState(today.getFullYear());
//   const [calMonth, setCalMonth] = useState(today.getMonth());
//   const [selectedDay, setSelectedDay] = useState(today.getDate());
//
//   // The `view` query param only matters for THRIFT + RENT listings — it lets
//   // the same product page render as either its thrift-adapted or
//   // rent-adapted variant without a full navigation.
...
//
//   // Seller info — falls back gracefully if product.seller isn't wired up yet
//   const sellerName = (product as any).seller?.name ?? "Anjali Sharma";
//   const sellerAvatarUrl = (product as any).seller?.avatarUrl ?? null;
//
//   // Flips between the thrift-adapted and rent-adapted view of THIS SAME
//   // listing via the `view` query param — no route change, no reload.
...
//
//   // Context-aware tag for THIS product on its own detail page (hybrid-safe)
...
//
//               {/* LEFT — Gallery (shorter hero image, thumbnail strip matched in width directly underneath) */}
//               <div className="space-y-2.5">
//                 {/* Main Container forcing the 4:3.3 aspect ratio */}
...
//                 </div>
//
//                 {/* Thumbnails Strip — Built to dynamically match the exact width of your 4/3.3 container */}
...
//                           {product.description ?? "This is the mindblowing magnificent dresses which is suitable for the weddings as well."}
//                         </p>
...
//                 {isThrift && (
//                     <>
//                       {/* Price + Trust card — single bordered card, no discount badge */}
...
//                           { label: "Availability", value: "Available", pill: true },
//                           { label: "Shipping / Pickup", value: "Flex (Both)" },
//                           { label: "Visible Flaws / Notes", value: "a small tripp but was fixed" },
//                         ].map(({ label, value, pill }) => (
//                             <div key={label} className="flex items-center justify-between text-[12px]">
//                               <span className="text-[#6E6053]">{label}</span>
//                               {pill ? (
//                                   <span className="rounded-full bg-[#E8F5EE] px-2.5 py-0.5 text-[11px] font-bold text-[#2E7D52]">{value}</span>
//                               ) : (
...
//                           </div>
//                           <div>
//                             <p className="text-[12px] font-medium text-[#4F4338]">a small tripp but was fixed</p>
...
//                           <span className="font-semibold text-[#1A130E]">{securityDeposit}</span>
//                         </div>
...
//                       </div>
//
//                       <button className="w-full rounded-lg bg-[#1A130E] py-3.5 text-[13px] font-bold text-white transition hover:bg-[#332620]">
//                         Rent Now
//                       </button>
//
...
//             </section>
//
//             {/* ══ TRUST STRIP — full-bleed, spans beyond both columns ══ */}
...
//           {items.map((item) => {
//             // Hybrid-aware tag: THRIFT+RENT items always show "THRIFT + RENT",
//             // colored red in thrift-context rows and grey in rent-context rows.
                    ...
                    <section className="grid gap-8 lg:grid-cols-[1fr_1fr] lg:items-start">
                        ...
                        <div className="space-y-2.5">
                            ...
                            <div className="pt-3">
                                ...
                                {/* ── Care Guide Tab ──
                     Static instructions — see lib/constants/care.ts.
                     Same content shown on every listing; backend has
                     no per-item care field yet. */}
                                {activeTab === "care" && (
                                    <div className="mt-4 rounded-xl border border-[#EBE3D5] bg-[#FDFAF6] p-5">
                                        <h4 className="text-[14px] font-bold text-[#1A130E]">Care Instructions</h4>
                                        <div className="mt-3 space-y-3">

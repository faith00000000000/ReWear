  ...
  useEffect(() => {
    ...
    const tick = () => {
      const startedAt = Number(
        localStorage.getItem(RESERVATION_STORAGE_KEY) ?? Date.now(),
      );
        ...
        <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
          ...
          {items.length > 0 && remainingMs !== null && (
            ...
            >
              <Clock
                size={14}
                className={isExpiringSoon ? "text-[#9E2A1B]" : "text-[#9E2A1B]"}
              />
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[#DDD5C8] bg-white py-24 text-center">
            ...
            <p className="mt-2 max-w-xs text-[13px] text-[#6E6053]">
              Explore our catalog to discover authentic, curated vintage
              garments.
            </p>
            <Link
              href="/browse-finds"
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1.8fr_1fr] lg:items-start">
            <div className="space-y-3">
              {items.map((item) => {
                ...
                const isRent =
                  item.status === "RENT" || item.status === "THRIFT + RENT";
                  ...
                  >
                    ...
                    <div className="flex flex-1 flex-col justify-between px-5 py-4">
                      <div>
                        ...
                        <div className="mt-2 flex items-start justify-between gap-3">
                          ...
                          <div className="shrink-0 text-right">
                            ...
                            <p
                              className={`text-[13px] font-bold ${item.deliveryFee === 0 ? "text-[#4A6B3A]" : "text-[#1A130E]"}`}
                            >
                              {feeValue}
                            </p>
                          </div>
                        ...
                        <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[12px] text-[#6E6053]">
                          <span>
                            Size:{" "}
                            <strong className="font-semibold text-[#1A130E]">
                              {item.size}
                            </strong>
                          </span>
                          {!isRent && (
                      ...
                      <div className="mt-3 flex items-end justify-between gap-2">
                        ...
                        {!isPickup && !isRent && <div className="flex-1" />}
                      </div>
                    </div>
                  </div>
                );
            ...
            <div className="sticky top-6 w-full rounded-2xl border border-[#EBE3D5] bg-[#FCFAF7] p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-2 border-b border-[#EBE3D5] pb-4">
                {items.map((item) => (
                  ...
                  >
                    <span className="truncate max-w-[170px] text-[#4F4338]">
                      {item.name}
                    </span>
                    <span className="shrink-0 font-semibold text-[#1A130E]">
                      {item.price}
                    </span>
                  </div>
                ))}
              ...
              <div className="mt-4 space-y-3 text-[14px]">
                ...
                <div className="flex items-baseline justify-between rounded-xl bg-[#FAF0E6] px-4 py-3 border border-[#EBE3D5]">
                  <span className="text-[15px] font-bold text-[#1A130E]">
                    Total
                  </span>
                  <span className="text-[24px] font-bold text-[#9E2A1B]">
                    {fmt(total)}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-4 flex items-start gap-3 rounded-xl border border-[#DCE4DA] bg-[#F3F6F2] px-4 py-3">
                ...
                <p className="text-[11px] leading-relaxed text-[#405A35]">
                  Great choice! You are saving pieces from landfill and
              ...
              <div className="mt-5 space-y-3 border-t border-[#EBE3D5] pt-4">
                {[
                  ...
                  },
                  {
                    ...
                    desc: "Quality-checked by our experts",
                  },
                  {
                    icon: RefreshCw,
                    title: "Easy returns",
                    desc: "14-day returns & exchanges",
                  },
                  ...
                  <div key={title} className="flex items-start gap-3">
                    <Icon
                      size={16}
                      strokeWidth={1.5}
                      className="mt-0.5 shrink-0 text-[#6E6053]"
                    />
                    <div>
              ...
              {[
                {
                  ...
                  desc: "Every purchase helps reduce waste and support slow fashion.",
                },
                {
                  icon: Shirt,
                  title: "Curated with Care",
                  desc: "Handpicked pieces with quality and authenticity.",
                },
                {
                  icon: ShieldCheck,
                  title: "Quality You Can Trust",
                  desc: "Inspected items. Fair pricing. Honest condition.",
                },
                {
                  icon: RefreshCw,
                  title: "Better for the Planet",
                  desc: "Extending the life of fashion, one find at a time.",
                },
                ...
                >
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="max-w-[190px] text-[11px] leading-relaxed text-[#7A6C5F]">
                      {desc}
                    </p>
                  </div>
                </div>
...
function PaymentModal({
  ...
  const [selectedPayment, setSelectedPayment] = useState<"esewa" | "khalti">(
    "esewa",
  );
  ...
  async function handlePay() {
    ...
    try {
      ...
      if (data.gateway === "ESEWA") {
        ...
        Object.entries(
          data.gatewayFormFields as Record<string, string>,
        ).forEach(([key, value]) => {
      ...
      >
        ...
        <div className="px-8 pt-5 pb-3 text-center">
          ...
          <p className="mt-0.5 text-[13px] text-[#6E6053]">
            Pay <span className="font-bold text-[#9E2A1B]">{fmt(total)}</span>{" "}
            securely online.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-[#EBE3D5] border-t border-[#EBE3D5] px-6 py-3">
          {[
            ...
            },
            {
              ...
              desc: "Every item is authenticated.",
            },
            {
              icon: RefreshCw,
              title: "Easy Returns",
            ...
            <div
              key={title}
              className="flex items-start gap-2.5 p-2 sm:px-4 first:pl-0 last:pr-0"
            >
              <Icon
                size={17}
                strokeWidth={1.6}
              ...
              <div>
                ...
                <p className="text-[10.5px] leading-snug text-[#6E6053]">
                  {desc}
                </p>
              </div>
            </div>

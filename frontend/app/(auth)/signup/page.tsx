...
const inputClass =
  "w-full rounded-[10px] border-0 bg-[#122e1e] px-4 py-3 [font-family:'Inter',sans-serif] text-[13.5px] text-white caret-[#22c55e] outline-none placeholder:text-[#6b9e80] focus:bg-[#0d2417]";

const strengthConfig = [
  { label: "Weak", bar: "bg-[#ef4444]", text: "text-[#ef4444]" },
  { label: "Fair", bar: "bg-[#f97316]", text: "text-[#f97316]" },
  { label: "Good", bar: "bg-[#eab308]", text: "text-[#eab308]" },
  { label: "Strong", bar: "bg-[#84cc16]", text: "text-[#84cc16]" },
  { label: "Very Strong", bar: "bg-[#22c55e]", text: "text-[#22c55e]" },
] as const;
...
function BrandLogo({ mobile = false }: { mobile?: boolean }) {
  return (
    <div className={`z-10 flex items-center gap-2.5 ${mobile ? "mb-6 lg:hidden" : "px-[18px] pt-[18px]"}`}>
      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-[#1a5c38] text-base text-white">
        ♻
      </div>
      <div>
        <p className={`[font-family:'Inter',sans-serif] text-xs font-extrabold tracking-[0.14em] ${mobile ? "text-white" : "text-[#1a3d26]"}`}>
          REWEAR
        </p>
        <p className={`[font-family:'Inter',sans-serif] text-[8.5px] font-semibold tracking-[0.12em] ${mobile ? "text-[#5aaa7a]" : "text-[#2e7d52]"}`}>
          SUSTAINABLE FASHION
        </p>
      </div>
    </div>
...
function LeftPanel() {
  return (
    <div className="relative hidden min-h-[640px] w-[360px] shrink-0 flex-col justify-between overflow-hidden bg-[#d6ede1] lg:flex">
...
function Field({
  ...
  return (
    <div className="relative">
      <input
        {...props}
        className={`${inputClass} ${rightSlot ? "pr-11" : ""} ${className}`}
      />
      {rightSlot ? (
...
export default function SignupPage() {
  ...
  const passwordStrength = formData.password ? calcStrength(formData.password) : 0;
  ...
  const eyeBtn = (show: boolean, toggle: () => void) => (
    <button
      ...
      className="flex cursor-pointer border-0 bg-transparent p-0 text-[#4e8a67]"
    >
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#1a4230]">
      ...
      <div className="relative z-10 flex w-[900px] max-w-[calc(100vw-48px)] items-stretch overflow-hidden rounded-[20px] shadow-[0_12px_60px_rgba(0,0,0,0.35)]">
        <LeftPanel />

        <div className="flex max-h-screen flex-1 flex-col justify-center overflow-y-auto bg-[#1e4a33] px-[52px] py-9">
          <BrandLogo mobile />
          <h1 className="mb-1 [font-family:'Inter',sans-serif] text-[38px] font-bold leading-[1.1] tracking-[-0.5px] text-white">
            Create Account
          </h1>
          <p className="mb-[22px] [font-family:'Inter',sans-serif] text-[13px] text-[#6aaa82]">
            Join ReWear and start sustainable shopping
          </p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
                ...
                <div className="mt-2">
                  <div className="mb-1 flex gap-1">
                    {Array.from({ length: 4 }).map((_, i) => (
                      <div
                        key={i}
                        className={`h-1 flex-1 rounded-full transition-colors ${i < passwordStrength ? strength.bar : "bg-[#1e4a31]"}`}
                      />
                    ))}
                  </div>
                  <p className="[font-family:'Inter',sans-serif] text-[11.5px] text-[#6aaa82]">
                    Strength:{" "}
            ...
            <div>
              ...
              <Field
                ...
                onChange={handleChange}
                required
                rightSlot={eyeBtn(showConfirmPassword, () =>
                  setShowConfirmPassword((p) => !p),
                )}
              />
              {formData.confirmPassword &&
              formData.password !== formData.confirmPassword ? (
                <p className="mt-[5px] [font-family:'Inter',sans-serif] text-[11.5px] text-[#f87171]">
            ...
            <div className="flex items-start gap-2.5">
              <input
                ...
                onChange={handleChange}
                className="mt-0.5 h-[15px] w-[15px] shrink-0 cursor-pointer accent-[#22c55e]"
              />
              <label
                htmlFor="agreeToTerms"
                className="cursor-pointer [font-family:'Inter',sans-serif] text-[12.5px] leading-[1.5] text-[#8ac4a4]"
              >
                I agree to the{" "}
                <Link href="/terms" className="font-semibold text-[#22c55e] no-underline">
                  Terms and Services
                </Link>{" "}
                and{" "}
                <Link href="/privacy" className="font-semibold text-[#22c55e] no-underline">
                  Privacy Policy
                </Link>
              </label>
            </div>
            ...
            <button
              ...
              className="mt-0.5 w-full cursor-pointer rounded-[10px] border-0 bg-[#22c55e] p-3.5 [font-family:'Inter',sans-serif] text-[15px] font-bold tracking-[0.02em] text-white transition-[filter] hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:brightness-100"
            >
          ...
          <p className="mt-5 text-center [font-family:'Inter',sans-serif] text-[13px] text-[#8ac4a4]">
            Already have an account?{" "}
            <Link href="/login" className="font-bold text-[#22c55e] no-underline">
              Login
            </Link>
          </p>

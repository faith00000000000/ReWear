...
/**
 * Where a given role should land after auth. Admins always go to /admin
 * regardless of any ?redirect= param — a redirect param is meant for
 * "take me back to the product page I was on," which doesn't apply to
 * an admin account.
 */
...
function LoginContent() {
  ...
  // Previously always sent already-authed users to redirectTo, ignoring
  // role — an admin bouncing back to /login (e.g. stale tab, bookmark)
  // would get dropped on the regular dashboard instead of /admin. Now
  // routes by role here too, same as the post-login-submit path below.
  useEffect(() => {
    if (!isMounted) return; // Wait until AuthContext finishes localStorage check

    if (authed) {
      router.replace(destinationFor(user?.role, redirectTo));
    }
  }, [isMounted, authed, user, router, redirectTo]);
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (loggedInUser) {
        saveSession({ accessToken, refreshToken, expiresIn }, loggedInUser);
      } else {
        ...
        <div className="relative z-10 w-full max-w-[500px] rounded-[28px] bg-white px-8 py-10 shadow-[0_12px_60px_rgba(33,23,20,0.03)] border border-[#F2ECE4] sm:px-10 sm:py-10">
          ...
          {error && (
            <div className="mt-4 flex items-start gap-2.5 rounded-lg bg-[#FDF3F2] border border-[#FADCD9] px-4 py-3">
              <AlertCircle
                size={15}
                className="text-[#EA4335] mt-0.5 shrink-0"
              />

    ...
    // ── Rename DistanceBucket → DeliveryZone (keep old name as alias so
    // nothing else in the codebase breaks) ──
    export type DeliveryZone = "DISTRICT" | "PROVINCE" | "NATIONWIDE";
    /** @deprecated use DeliveryZone — kept for backward compatibility */
    export type DistanceBucket = DeliveryZone;
    ...
    // ── NEW — zone matching by district/province text, not lat/lng ──
    // Seller side gives sellerDistrict/sellerProvince as plain text at
    // listing time (no map). Buyer side reverse-geocodes their pinned
    // location into a district/province and we compare the two here.
    export function resolveZoneFromLocation(
        sellerDistrict: string | undefined,
        sellerProvince: string | undefined,
        buyerDistrict: string | undefined,
        buyerProvince: string | undefined,
    ): DeliveryZone {
        const norm = (s?: string) => (s ?? "").trim().toLowerCase();
    
        if (norm(sellerDistrict) && norm(sellerDistrict) === norm(buyerDistrict)) {
            return "DISTRICT";
        }
        if (norm(sellerProvince) && norm(sellerProvince) === norm(buyerProvince)) {
            return "PROVINCE";
        }
        return "NATIONWIDE";
    }